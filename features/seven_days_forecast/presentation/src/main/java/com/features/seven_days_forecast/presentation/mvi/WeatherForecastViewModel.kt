            ...
            getDailyWeatherUseCase(cityId).collect{ resource ->
                withContext(Dispatchers.Main){
                    when(resource){
                        ...
                        is Resource.Success -> _state.value = WeatherForecastViewState.Success(resource.data)
                        is Resource.Failure -> _state.value = WeatherForecastViewState.Error(resource.exception.message ?: "Unknown error")
                    }
                }
            }
        }
