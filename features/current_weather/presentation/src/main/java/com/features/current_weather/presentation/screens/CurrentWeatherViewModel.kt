...
class CurrentWeatherViewModel @Inject constructor(
    ...
    private val _state : MutableState<CurrentWeatherViewState> = mutableStateOf(CurrentWeatherViewState.Loading)
            ...
            fetchCityDetailsUseCase(cityId).collect { resource ->
                withContext(Dispatchers.Main) {
                    when (resource) {
                        ...
                        is Resource.Failure -> {
                            _state.value = CurrentWeatherViewState.Error(resource.exception.message ?: "Unknown error")
                        }
                    }
                }
